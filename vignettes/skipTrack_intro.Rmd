...
```{r}
#Simulate data with no true covariates
...
ft <- skipTrack.fit(Y = dat$Y, cluster = dat$cluster, X = dat$X, Z = dat$Z,
                    reps = 2000, chains = 4, useParallel = TRUE)
```
...
A summary is available for the SkipTrack model fit with `summary(ft)`, with more detailed results accessible through `skipTrack.results(ft)`.
