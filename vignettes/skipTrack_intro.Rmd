...
```{r}
#Simulate data with no covariates
dat <- skipTrack.simulate(n = 500, model = 'skipTrack', skipProb = c(.75, .2, .05))

#Simulate data with covariates for the mean (Beta) and precision (Gamma)
dat2 <- skipTrack.simulate(n = 500, model = 'skipTrack', skipProb = c(.75, .2, .05),
                           trueBetas = c(.06, 0), trueGammas = c(.3, 0))

names(dat)
```

The result of the simulation function is simply a named list with various components. These are 
...
In this code we ask for 4 chains, each with 2000 iterations, run in parallel.

```{r}
ft <- skipTrack.fit(Y = dat$Y, cluster = dat$cluster, X = dat$X, Z = dat$Z,
                    reps = 4000, chains = 4, useParallel = TRUE)
```
...
Multivariate, multichain MCMC diagnostics, including traceplots, Gelman-Rubin diagnostics, and effective sample size, are all available for various parameters from the model fit. These are supplied using the `genMCMCDiag` package, see CITATION for details. 

Here we show the output of the diagnostics on the $c_{ij}$ parameters, which show that (at least for the $c_{ij}$ values) the algorithm is mixing effectively.
...
summary(ft, burnIn = 1000)
```

This introduction provides enough information to start fitting the SkipTrack model. For further information regarding different methods of simulating data and tuning parameters for fitting the model, please see the help pages and additional vignettes. 
