#This document contains functions necessary to perform hyperparameter inference with
#the method described in Li et al. 2022


#' Calculate the likelihood of parameters for an individual's cycle lengths
#'
#' This function calculates the likelihood of the parameters for a given individual's data.
#' It is adapted from a formula provided in Li et al. (2022)
#'
#' @param di Numeric vector representing individual data.
#' @param lambdai Numeric value representing the individuals cycle length mean parameter
#' @param pii Numeric value representing the individuals probability of skipping tracking a cycle.
#' @param S Maximum number of allowed skips in the model.
#'
#' @return Numeric value representing the likelihood of the parameters for the data.
#'
#' @details
#' The function calculates the likelihood for an individuals data based on
#' the formula provided in the Supplementary Materials by Li et al. (2022).
#'
#' @references
#' Li, Kathy, et al. "A predictive model for next cycle start date that accounts for
#' adherence in menstrual self-tracking." Journal of the American Medical
#' Informatics Association 29.1 (2022): 3-11.
#'
#'
liki <- function(di, lambdai, pii, S){
  s <- 0:S
  #Get sum portion of likelihood (as a vector)
  icSums <- sapply(di, function(dic){
    return(sum(((s+1)^dic)*(pii*exp(-lambdai))^s))
  })

  likic <- (1-pii)/(1-pii^(S+1))*dpois(di, lambdai)*icSums
  return(exp(sum(log(likic))))
}

#' Monte Carlo estimate of negative marginal log-likelihood of Li model
#'
#' This function calculates a Monte Carlo estimate of the negative marginal log-likelihood
#' of the given hyperparameters for the generative model from Li et al. (2022).
#' It samples M instances of the parameters from the given distributions
#' and averages the results from the likelihood function \code{\link{liki}}.
#'
#' @param pars Named numeric vector of hyperparameters containing the
#' elements: kappa, gamma, alpha, beta.
#'   - \code{kappa}: Numeric value, shape parameter of Gamma distribution for Lambda_i.
#'   - \code{gamma}: Numeric value, rate parameter of Gamma distribution for Lambda_i.
#'   - \code{alpha}: Numeric value, shape1 parameter of Beta distribution for Pi_i.
#'   - \code{beta}:  Numeric value, shape2 parameter of Beta distribution for Pi_i.
#' @param S Integer, maximum number of allowed skips in the model.
#' @param M Integer specifying the number of Monte Carlo iterations.
#' @param cycleDat Data frame containing information about individuals and their tracked cycles.
#'
#' @return Numeric value representing the Monte Carlo estimate of the negative marginal log-likelihood.
#'
#' @details
#' The function simulates parameters for each individual and iteration, calculates the likelihoods
#' using the \code{\link{liki}} function, and averages the results over the iterations.
#'
#' @seealso
#' \code{\link{liki}} for the likelihood calculation function.
#'
likm <- function(pars = c(kappa = 180,
                             gamma = 6,
                             alpha = 2,
                             beta = 20), S, M = 1000, cycleDat, verbose = TRUE, ...){
  kappa <- max(pars[1], 0)
  gamma <- max(pars[2], 0)
  alpha <- max(pars[3], 0)
  beta <- max(pars[4], 0)

  uniqueInds <- unique(cycleDat$Individual)
  #Simulate parameters, one for each individual, per M iteration
  numIn <- length(uniqueInds)

  #Simulate means(returns numIn x M table)
  lamIs <- sapply(1:M, function(m){
    rgamma(numIn, shape = kappa, rate = gamma)
  })

  #lamIs <- rgamma(M, shape = kappa, rate = gamma)

  #Simulate skip probs (returns numIn x M table)
  piIs <- sapply(1:M, function(m){
    rbeta(numIn, shape1 = alpha, shape2 = beta)
  })

  #piIs <- rbeta(M, shape1 = alpha, shape2 = beta)

  #Iterate through individuals, calculating all likelihoods
  liIs <- sapply(1:numIn, function(i){
    ind <- uniqueInds[i]
    ds <- cycleDat$TrackedCycles[cycleDat$Individual == ind]
    sapply(1:M, function(m){
      return(liki(di = ds,
                  lambdai = lamIs[i,m], pii = piIs[i,m], S = S))
    })
  })

  #liICs <- sapply(1:M, function(m){
  #  return(liki(di = cycleDat$TrackedCycles, lambdai = lamIs[m], pii = piIs[m], S = S))
  #})

  if(verbose){
    print(paste0('Current Value: ', sum(-log(colMeans(liIs)))))
  }
  #Average over M and return - log lik of sum
  return(sum(-log(colMeans(liIs))))
}

#' Monte Carlo estimate of negative marginal log-likelihood of Li model
#'
#' This function calculates a Monte Carlo estimate of the negative marginal log-likelihood
#' of the given hyperparameters for the generative model from Li et al. (2022).
#' It samples M instances of the parameters from the given distributions
#' and averages the the likelihoods, giving a marginal likelihood for the hyperparameters.
#'
#' @param pars Named numeric vector of hyperparameters containing the
#' elements: kappa, gamma, alpha, beta. NOTE: MUST BE IN CORRECT ORDER.
#'   - \code{kappa}: Numeric value, shape parameter of Gamma distribution for Lambda_i.
#'   - \code{gamma}: Numeric value, rate parameter of Gamma distribution for Lambda_i.
#'   - \code{alpha}: Numeric value, shape1 parameter of Beta distribution for Pi_i.
#'   - \code{beta}:  Numeric value, shape2 parameter of Beta distribution for Pi_i.
#' @param S Integer, maximum number of allowed skips in the model.
#' @param M Integer specifying the number of Monte Carlo iterations.
#' @param cycleDat Data frame containing information about individuals and their tracked cycles.
#'
#' @return Numeric value representing the Monte Carlo estimate of the negative marginal log-likelihood.
#'
likVec <- function(pars = c(kappa = 180,
                            gamma = 6,
                            alpha = 2,
                            beta = 20), S, M = 1000, cycleDat, verbose = TRUE, ...){
  if(verbose){
    message(
      paste0('\nParameters: ', pars)
    )
  }
  kappa <- max(pars[1], 0)
  gamma <- max(pars[2], 0)
  alpha <- max(pars[3], 0)
  beta <- max(pars[4], 0)

  #Simulate parameters one per M iteration
  numCyc <- nrow(cycleDat)

  lamIs <- rgamma(M, shape = kappa, rate = gamma)
  piIs <- rbeta(M, shape1 = alpha, shape2 = beta)

  #Get s+1 raised to cycles
  sTOcyc <- rep(1:(S+1), times = rep(numCyc, S+1))^cycleDat$TrackedCycles
  sTOcyc <- rep(sTOcyc, M)[order(rep(rep(1:(S+1), times = rep(numCyc, S+1)), M))]

  #Get pi*exp(-lam)^S
  pilamTOs <- (piIs*exp(-lamIs))^rep(0:S, times = rep(M, S+1))
  pilamTOs <- rep(pilamTOs, times = rep(numCyc, M*(S+1)))

  #Repeat cycle vec M times, and lambda/pi unique pairs, cycle number of times
  cycM <- rep(cycleDat$TrackedCycles, M)
  lamM <- rep(lamIs, times = rep(numCyc, M))
  piM <- rep(piIs, times = rep(numCyc, M))

  #TRY 3, calculate first half, then repeat to calculate second half, then combine
  firstHalf <- ((1-piM)/(1-piM^(S+1)))*dpois(cycM, lamM)

  secondHalf <- rowSums(matrix(pilamTOs*sTOcyc, ncol = S+1))

  lcijmLiks <- log(firstHalf*secondHalf)


  #Now, combine over j
  individualSplits <- as.logical(diff(rep(cycleDat$Individual, M)))
  dimLiks <- exp(diff(c(0,
                        cumsum(lcijmLiks)[individualSplits],
                        sum(lcijmLiks))))

  #Average over Ms
  diLiks <- rowMeans(matrix(dimLiks, ncol = M))

  #Return sum of negative log likelihoods per individual
  negLLik <- -sum(log(diLiks))

  if(verbose){
    message(paste0('\nCurrent Value: ',negLLik))
  }

  return(negLLik)
}

#This function performs hyperparameter inference on a given dataset of individuals and
#their tracked cycles, assuming the model specified in Li et al. (2022). Default starting values
#for hyperparameters and optimization tuning parameters are those
#given in Li et al.
liInference <- function(cycleDat, S = 10,
                        startingParams = c(kappa = 180,
                                              gamma = 6,
                                              alpha = 2,
                                              beta = 20)){
  #Perform inference using ADAM
  ret <- optimg::optimg(par = startingParams, fn = likVec, gr = NULL, S = S, cycleDat = cycleDat,
                        method = 'ADAM', control = list(reltol = .001),
                        maxit = 1000, tol = 1e-3, Interval = .01, verbose = TRUE)
  View(ret)
}
